...
import type { RouletteCategory } from '@/types/roulette';
...
export function RouletteWheel({ categories, selectedCategory, onSpin, spinning }: RouletteWheelProps) {
  const [rotation, setRotation] = useState(0);
  const wheelRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (spinning && selectedCategory) {
      // Calcular a posição da categoria selecionada
      const selectedIndex = categories.findIndex(cat => cat.category === selectedCategory);

      if (selectedIndex !== -1) {
        // Calcular ângulo para parar na categoria selecionada
  ...
  }, [spinning, selectedCategory, categories]);
  ...
  return (
    <div className="flex flex-col items-center gap-8">
          ...
          {categories.map((category, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span className="text-sm drop-shadow-md line-clamp-2">
                    {category.label}
                  </span>
                </div>
              </div>
      ...
      {selectedCategory && !spinning && (
        <div className="bg-muted border-2 border-primary rounded-lg p-6 text-center">
